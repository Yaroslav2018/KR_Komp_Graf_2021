...
void Graphics::DrawLineBresenham(int16_t x1, int16_t y1, int16_t x2, int16_t y2, int16_t sym, int16_t col)
{
	...
	*/
	...
	{
		deltaY <<= 1;
		balance = deltaY - deltaX;		// f(sssssssM)
		deltaX <<= 1;
...
{
	float fFovRad = 1.0f / tanf(fFovDegrees * 0.5f / 180.0f * PI); // Считаем тангенс угла обзора. Предварительно переводим его в радианы из градусов.
	mat4x4 matrix;  // Делим на 1, чтобы при увеличении угла обзора фигуры уменьшались и их помещалось больше на экране
	matrix.m[0][0] = fAspectRatio * fFovRad;// Меняем масштаб фигур по X относительно угла обзора 
                              //(Если не умножать на соотноешния сторон, то фигуры будут растянуты по оси X)
	matrix.m[1][1] = fFovRad; // Меняем масштаб фигур по Y относительно угла обзора
	matrix.m[2][2] = fFar / (fFar - fNear);  // Нормализуем область по Z от Fnear до Ffar
	matrix.m[3][2] = (-fFar * fNear) / (fFar - fNear); // Добавляем к предыдущему выражению часть от Наблюдателя до Fnear
	matrix.m[2][3] = 1.0f; // Ставим 1 для сохранения в w значения Z, в дальнейшем будем на него делить
	matrix.m[3][3] = 0.0f;
	return matrix;
}

Graphics::mat4x4 Graphics::Matrix_MakeIsometricProjection()
{
